...
getPHPInfo() {
    ...
    phpThreadSafetyStatus=${detectedPHPThreadSafetyStatus}
...
detectPHPInfo() {
    ...
    detectedPHPThreadSafetyStatus=$(getPHPThreadSafetyStatus "${__phpBinary}")
...
resolvePHPInfo() {
    ...
    phpThreadSafetyStatus=${phpThreadSafetyStatus:-${detectedPHPThreadSafetyStatus}}
    phpSuhosinPatchStatus=${detectedPHPSuhosinPatchStatus}
}
...
getPHPThreadSafetyStatus() {
    local phpProgram="$1" ; shift
    local ThreadSafetyStatus
    ThreadSafetyStatus=$(${phpProgram} -dagent.cli_enabled=0 -r 'print "PHP-ThreadSafetyStatus:" . (PHP_ZTS ? "ZTS" : "NTS") . "\n";' 2>/dev/null)
    ThreadSafetyStatus=$(echo "${ThreadSafetyStatus}" | grep -e "^PHP-ThreadSafetyStatus:" | sed "s/PHP-ThreadSafetyStatus://")
    echo ${ThreadSafetyStatus}
}
